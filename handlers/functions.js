...
async function swap_pages2(client, message, interaction, embeds) {
  let currentPage = 0;
 ...
 new MessageButton()
  ...
  const collector = interaction.channel.createMessageComponentCollector({ filter, time: 1000000000 });
  ...
  collector.on("collect", async (i) => {
      ...
      if (i.customId == "next") {
        ...
        collector.on("end", (_, reason) => {
          try { idk.delete() } catch {/** */}
          if (reason === "time") {
            const embed = new MessageEmbed()
            .setColor(ee.wrongcolor)
            .setDescription(`${emoji.msg.ERROR} Time's up! Please press the button again`)
            return message.channel.send({embeds: [embed]}).then(msg => setTimeout(() => { msg.delete() }, 3000));
          }
        });
}
